...
const generateDummyEventLogs = (count: number = 2000): EventLog[] => {
    const eventTypes = [
        "LOGIN", "ADMIN_CREATE", "PASSWORD_CHANGE",
    ...
    for (let i = 0; i < count; i++) {
        ...
        const createdAt = new Date(baseTime.getTime() - (randomDays * 24 * 60 * 60 * 1000) - (randomHours * 60 * 60 * 1000) - (randomMinutes * 60 * 1000));

        const eventType = eventTypes[Math.floor(Math.random() * eventTypes.length)];
        const result = results[Math.floor(Math.random() * results.length)];
        const adminUsername = adminUsernames[Math.floor(Math.random() * adminUsernames.length)];
        const userId = userIds[Math.floor(Math.random() * userIds.length)];

        // 이벤트 타입에 따른 상세 정보 생성
        const eventDetail = generateEventDetail(eventType, adminUsername, userId, result);
    ...
    // 시간순 정렬 (최신순)
    dummyLogs.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    return dummyLogs;
};

const generateEventDetail = (eventType: string, adminUsername: string, userId: string | null, result: string): string => {
    if (eventType === "LOGIN") {
        if (result === "SUCCESS") {
        ...
        if (result === "SUCCESS") {
            return userId ? `Users created successfully: [${userId}]` : "Users created successfully: [user001, user002]";
        } else {
            return userId ? `Failed to create users - duplicates found: [${userId}]` : "Failed to create users - duplicates found: [user001]";
        }
    } else if (eventType === "USER_UPDATE") {
        if (result === "SUCCESS") {
            return userId ? `User updated successfully: ${userId}` : "User updated successfully: user001";
        } else {
            return userId ? `Failed to update user - user not found: ${userId}` : "Failed to update user - user not found: user001";
        }
    } else if (eventType === "USER_DELETE") {
        if (result === "SUCCESS") {
            return userId ? `Users deleted successfully: [${userId}]` : "Users deleted successfully: [user001, user002]";
        } else {
            return userId ? `Failed to delete users - users not found: [${userId}]` : "Failed to delete users - users not found: [user001]";
        }
    } else {
    ...
    const fetchEventLogs = async () => {
        ...
        try {
            if (DEBUG) {
                ...
                const response = await authenticatedFetch(`/event-logs?${params.toString()}`);
    ...
    const applyFilters = () => {
        if (DEBUG) {
            ...
            if (filters.admin_username) {
                filteredLogs = filteredLogs.filter(log => log.admin_username === filters.admin_username);
            }
            if (filters.user_id) {
                filteredLogs = filteredLogs.filter(log => log.user_id?.toString() === filters.user_id);
            }
        ...
        <Container maxWidth="xl" sx={{ py: 4 }}>
            ...
            {eventLogs.length >= 2000 && (
                <Alert severity="warning" sx={{ mb: 3 }}>
                    이벤트 로그가 최대 표시 개수(2,000개)에 도달했습니다. 더 많은 로그를 확인하려면 필터를 사용하여 검색 범위를 좁혀주세요.
                </Alert>
            )}
