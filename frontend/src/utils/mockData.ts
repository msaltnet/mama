...
// 랜덤 배열 요소 선택 함수
function getRandomArrayElements<T>(
  array: T[],
  minCount: number,
  maxCount: number,
): T[] {
...
function generateRandomUser(id: number): User {
  ...
  const extraInfo =
    Math.random() > 0.5
      ? `Test user ${id} - ${Math.random().toString(36).substring(7)}`
      : undefined;
  ...
  const updatedDate =
    ...
    Math.random() * (Date.now() - new Date(createdDate).getTime());
...
export function generateMockUsersFromIds(
  userIds: string[],
  organization?: string,
  extraInfo?: string,
  ...
  return userIds.map((userId) => ({
    ...
    allowed_models:
      allowedModels || getRandomArrayElements(AVAILABLE_MODELS, 1, 3),
